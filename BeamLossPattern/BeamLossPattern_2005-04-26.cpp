...
double CheckPos(double pp, double ringLenght); // See CheckPos.cpp
...
int main (int argc, char* argv[])
{
  ...
  //Added by MF, 12/06/2015 to adapt code to FCC
  char chr_tmp[256];
  string str_tmp;
  char tname[256], t[256], tT[256], tV[256];

  ifstream TwissFile(Twiss.c_str());
  while (TwissFile.getline(chr_tmp,256)){
    str_tmp = chr_tmp;
    if (str_tmp.find("LENGTH",0) != string::npos) {
      cout << "Input string is " << chr_tmp << endl;
      sscanf(chr_tmp,"%s%s%s%s",tname,t,tT,tV);
      LHCLength = atof(tV);
      break;}
  }
  cout<<"Summary of input parameters Ring lenght:" << LHCLength << endl;
  ...
  while (1){
	...
	if ( OldS[n_t] < 0.0 ){
	  ...
	  while ( new_s_t < s_t && 
	    ...
	    new_s_t = CheckPos( new_s_t + Dl, LHCLength);
	    new_x_t = x_t - (s_t-new_s_t) * OldXP[n_t];
	    new_y_t = y_t - (s_t-new_s_t) * OldYP[n_t];
	    if ( _S_ )
...
double CheckPos (double pp, double ringLenght)
{
  double LHCLength = ringLenght; //26658.8832;
  if ( pp < 0.0 )
    pp = LHCLength + pp;
  if ( pp > LHCLength )
    pp = pp - LHCLength;
